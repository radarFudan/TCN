...
parser = argparse.ArgumentParser(
    description="Sequence Modeling - Word-level Language Modeling"
)

parser.add_argument(
    "--batch_size", type=int, default=16, metavar="N", help="batch size (default: 16)"
)
...
parser.add_argument(
    "--emsize", type=int, default=600, help="size of word embeddings (default: 600)"
)
...
parser.add_argument(
    "--validseqlen", type=int, default=40, help="valid sequence length (default: 40)"
)
parser.add_argument(
    "--seq_len",
    type=int,
...
weight = torch.ones((1, args.seq_len, 1))
if args.power <= 99:
    for i in range(1, args.seq_len):
        weight[0, i, 0] = (i / args.seq_len) ** args.power
else:
    for i in range(1, args.seq_len):
        weight[0, i, 0] = 1
...
def train():
    ...
    for batch_idx, i in enumerate(range(0, train_data.size(1) - 1, args.validseqlen)):
        ...
        print(final_target.shape, final_output.shape)
        exit()
        weighted_out = final_output * weight[:, : final_output.shape[1]]
        loss = criterion(weighted_out.view(-1, n_classes), final_target.view(-1))
