...
parser = argparse.ArgumentParser(
    description="Sequence Modeling - LAMBADA Textual Understanding"
)
parser.add_argument(
...
parser.add_argument("--ksize", type=int, default=4, help="kernel size (default: 4)")
parser.add_argument(
    "--data",
    type=str,
    default="./data/lambada",
    help="location of the data corpus (default: ./data/lambada)",
)
parser.add_argument(
    "--emsize", type=int, default=500, help="size of word embeddings (default: 500)"
)
parser.add_argument("--levels", type=int, default=5, help="# of levels (default: 5)")
parser.add_argument(
    "--log-interval",
    type=int,
    default=100,
...
parser.add_argument(
    "--validseqlen", type=int, default=50, help="valid sequence length (default: 50)"
)
parser.add_argument(
    "--seq_len",
    type=int,
...
parser.add_argument(
    "--corpus", action="store_true", help="force re-make the corpus (default: False)"
)
parser.add_argument(
    "--power",
    type=int,
...
def evaluate(data_source):
    ...
    with torch.no_grad():
        for i in range(len(data_source)):
            data, targets = torch.LongTensor(data_source[i]).view(
                1, -1
            ), torch.LongTensor([data_source[i][-1]]).view(1, -1)
...
def train():
    ...
    for batch_idx, i in enumerate(range(0, train_data.size(1) - 1, args.validseqlen)):
        ...
        final_output = output[:, eff_history:].contiguous()

        # Time weighted loss
        weight = torch.ones((1, final_output.shape[1], 1), device=final_output.device)
        for i in range(1, final_output.shape[1]):
